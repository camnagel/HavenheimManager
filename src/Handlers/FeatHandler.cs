...
public class FeatHandler
{
    ...
    private readonly HashSet<Bonus> BonusFeatFilters = new();
    private readonly HashSet<Class> ClassFeatFilters = new();
    private readonly HashSet<Combat> CombatFeatFilters = new();
    private readonly HashSet<Condition> ConditionFeatFilters = new();

    // Feat Filter Lists
    private readonly HashSet<Core> CoreFeatFilters = new();
    private readonly HashSet<string> CustomFeatFilters = new();
    private readonly HashSet<Magic> MagicFeatFilters = new();
    private readonly HashSet<Role> RoleFeatFilters = new();
    private readonly HashSet<Skill> SkillFeatFilters = new();
    private readonly HashSet<Source> SourceFeatFilters = new();
    ...
    internal void ApplyFeatFilters()
    {
        ...
        List<Feat> possibleFeats =
            (_vm.FeatSearchText != RegexHandler.SearchPlaceholderText && _vm.FeatSearchText != ""
                ...
                : _vm.MasterFeatList).Where(x => !_vm.FavoriteFeatList.Contains(x) && !_vm.HiddenFeatList.Contains(x) &&
                                                 x.Level <= int.Parse(_vm.FeatMaxLevel) &&
                                                 x.Level >= int.Parse(_vm.FeatMinLevel)).ToList();
    ...
    internal void EditFeatAction(object arg)
    {
        try
        {
            ...
            {
                FeatViewModel vm = new(_vm.SelectedFeat);
